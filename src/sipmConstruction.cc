...
sipmConstruction::sipmConstruction(DBLinkPtr table, G4LogicalVolume *mother) : PMTConstruction("sipm") {
  glass_phys = 0;
  vacuum_phys = 0;
  ...
  Log::Assert(fParams.width > 0, "sipmConstruction: " + pmt_model + " width must be postive");
  Log::Assert(fParams.thick > 0, "sipmConstruction: " + pmt_model + " thick must be postive");
  Log::Assert(fParams.glassThickness > 0, "sipmConstruction: " + pmt_model + " glass thickness must be postive");
  Log::Assert(fParams.glass, "sipmConstruction: " + pmt_model + " has an invalid glass material");
  Log::Assert(fParams.vacuum, "sipmConstruction: " + pmt_model + " has an invalid vacuum material");
  Log::Assert(fParams.photocathode, "sipmConstruction: " + pmt_model + " has an invalid photocathode material");
}

G4LogicalVolume *sipmConstruction::BuildVolume(const std::string &prefix) {
  if (log_pmt) {
    return log_pmt;
  }
...
G4VSolid *sipmConstruction::BuildSolid(const std::string &name) {
